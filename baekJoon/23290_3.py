...
def move_fish():
    global arr
    temp_arr = {(i,j): [0]*8 for i in range(4) for j in range(4)}
    for i, j in arr:
...
def shark_dfs(x, y, depth, fish_cnt,path, arr):
    global candidates
    dx = [0, -1, 0, 1, 0]
    dy = [0, 0, -1, 0, 1]
    if depth == 3:
    ...
    for i in range(1,5):
        ...
        if 0 <= nx < 4 and 0 <= ny < 4:
            temp_arr=dict()
            for r, c in arr:
              temp_arr[(r,c)] = [item for item in arr[(r,c)]] # 처음에는 습관적으로 i, j를 썼다가 방향의 i랑 참서돼서 제대로 안돌아갔음
            temp_arr[(nx,ny)] = [] # 이미 먹힌 물고기가 더해지지 않도록
            shark_dfs(nx, ny, depth+1, fish_cnt+len(arr[(nx,ny)]), path+str(i), temp_arr)
    return
...
def main():
    ...
    sys.stdin = open('test_23290.txt', 'r')
